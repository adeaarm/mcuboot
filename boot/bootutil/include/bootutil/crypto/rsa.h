 ...
 * MCUBOOT_USE_PSA_CRYPTO. Since MCUBOOT_USE_PSA_CRYPTO does not yet
...
static inline int bootutil_rsa_oaep_decrypt(bootutil_rsa_context *ctx, size_t *olen, const uint8_t *input, uint8_t *output, size_t output_max_len)
{
 ...
 *
 * The key is meant to be used for OAEP decrypt hence those values are hardcoded
 */
static int
 ...
 *
 * The key is meant to be used for PSS signature verification hence those values are hardcoded
 */
static int
